...
function renderTable() {
    ...
    const entries = Object.values(users);
    if (entries.length === 0) {
        tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:48px;color:#52525b;font-size:13px;">No members found. Add your first member above.</td></tr>`;
    ...
    tbody.innerHTML = entries.map(u => {
        ...
        return `<tr style="border-bottom:1px solid rgba(255,255,255,0.04);">
            <td style="padding:16px 20px;font-family:'JetBrains Mono',monospace;font-size:11px;color:#52525b;">#${u.id}</td>
            <td style="padding:16px 20px;"><div style="font-size:14px;font-weight:600;color:#fff;margin-bottom:2px;">${u.name}</div><div style="font-size:11px;color:#52525b;">${u.role}</div></td>
            <td style="padding:16px 20px;">${tierBadge(u.tier)}</td>
            <td style="padding:16px 20px;font-family:'JetBrains Mono',monospace;font-size:11px;color:#a1a1aa;">${u.clearance}</td>
...
function showLoading() {
    document.getElementById('user-tbody').innerHTML = `<tr><td colspan="8" style="text-align:center;padding:48px;color:#52525b;font-size:13px;">Loading members from Firestore…</td></tr>`;
    document.getElementById('member-count').textContent = '…';
}

function showFetchError(msg) {
    document.getElementById('user-tbody').innerHTML = `<tr><td colspan="8" style="text-align:center;padding:48px;"><div style="color:#ef4444;font-size:13px;font-weight:600;margin-bottom:8px;">Firebase Error</div><div style="color:#52525b;font-size:12px;">${msg}</div></td></tr>`;
...
async function saveUser() {
    ...
    try {
        ...
        if (isNew && email) {
            ...
            if (error) console.warn('Supabase invite warning:', error.message);
        }
