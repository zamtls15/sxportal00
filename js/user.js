...
function showProfileNotSetup(email) {
    ...
    document.querySelector('.portal-container').innerHTML = `
        <div style="text-align:center;padding:80px 20px;animation:slideUp 0.8s cubic-bezier(0.16,1,0.3,1);">
            ...
            <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:#52525b;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:16px;">PROFILE PENDING</div>
            <h2 style="font-size:20px;font-weight:600;margin-bottom:12px;color:#ffffff;letter-spacing:-0.02em;">Profile Not Set Up Yet</h2>
            <p style="color:#52525b;font-size:13px;line-height:1.6;margin-bottom:32px;">
                ...
                <code style="color:#a1a1aa;background:rgba(255,255,255,0.05);padding:2px 10px;border-radius:4px;display:inline-block;">${email}</code>
            </p>
            <button data-action="not-member" style="display:inline-flex;align-items:center;gap:8px;padding:12px 24px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.08);border-radius:12px;color:#a1a1aa;font-size:13px;font-weight:600;cursor:pointer;font-family:'Inter',sans-serif;">
                Sign out
            </button>
        </div>`;
}
...
function showNotFound() {
    ...
    document.querySelector('.portal-container').innerHTML = `
        <div style="text-align:center;padding:80px 20px;">
            ...
            <h2 style="font-size:20px;font-weight:600;margin-bottom:12px;color:#ffffff;">Member Not Found</h2>
            <p style="color:#52525b;font-size:13px;">No record found for ID: <code style="color:#a1a1aa;background:rgba(255,255,255,0.05);padding:2px 8px;border-radius:4px;">${userId || 'none'}</code></p>
        </div>`;
}
...
function showFetchError() {
    ...
    document.querySelector('.portal-container').innerHTML = `
        <div style="text-align:center;padding:80px 20px;">
            ...
            <h2 style="font-size:20px;font-weight:600;margin-bottom:12px;color:#ffffff;">Service Unavailable</h2>
            <p style="color:#52525b;font-size:13px;">Could not reach the member database.<br>Check your connection and try again.</p>
        </div>`;
}
